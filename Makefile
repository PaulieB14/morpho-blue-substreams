...
.PHONY: all build pack pack-base protogen run stale vendor base-manifest clean
...
base-manifest:
	python3 -c "import re;s=open('substreams.yaml').read();\
...
open('substreams.base.yaml','w').write('# GENERATED from substreams.yaml by \`make base-manifest\`\n'+s)"

run: pack
	substreams run $(SPKG) db_out -e $(ENDPOINT) --start-block 18883124 --stop-block +1000

clean:
	cargo clean && rm -f $(SPKG)
